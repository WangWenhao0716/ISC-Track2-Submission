...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, pretrained=True, cut_at_pooling=False,
        ...
        if not self.cut_at_pooling:
            ...
            if self.num_classes > 0:
                ...
                self.classifier_0_1 = nn.Linear(256, self.num_classes//4, bias=False).to(self.dev[0])
                init.normal_(self.classifier_0_1.weight, std=0.001)
                self.classifier_1_1 = nn.Linear(256, self.num_classes//4, bias=False).to(self.dev[1])
                init.normal_(self.classifier_1_1.weight, std=0.001)
                self.classifier_2_1 = nn.Linear(256, self.num_classes//4, bias=False).to(self.dev[2])
                init.normal_(self.classifier_2_1.weight, std=0.001)
                self.classifier_3_1 = nn.Linear(256, self.num_classes//4, bias=False).to(self.dev[3])
    ...
    def forward(self, x, feature_withbn=False):
        ...
        bn_x_1 = torch.nn.functional.torch.nn.parallel.data_parallel(self.matrix_feat_bn, bn_x, device_ids=[0, 1, 2, 3])
        ...
        prob_1 = [None for _ in range(4)]
        prob_1[0] = (self.classifier_0_1(bn_x_1.to(self.dev[0]))).to(self.dev[0])
        prob_1[1] = (self.classifier_1_1(bn_x_1.to(self.dev[1]))).to(self.dev[0])
        prob_1[2] = (self.classifier_2_1(bn_x_1.to(self.dev[2]))).to(self.dev[0])
        prob_1[3] = (self.classifier_3_1(bn_x_1.to(self.dev[3]))).to(self.dev[0])
        prob_1 = torch.cat(prob_1, dim = 1)
        
        # <-Split FC
        return x, prob, prob_1
